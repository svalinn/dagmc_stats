...
def test_get_native_ranges():
    ...
    assert(vertex_range == native_ranges[0], triangle_range ==
           native_ranges[2], entityset_range == native_ranges[11])
...
def test_get_entityset_ranges():
    ...
    node_range = my_core.get_entities_by_type_and_tag(
        root_set, types.MBENTITYSET, dagmc_tags['geom_dim'], [0])
    curve_range = my_core.get_entities_by_type_and_tag(
        root_set, types.MBENTITYSET, dagmc_tags['geom_dim'], [1])
    surface_range = my_core.get_entities_by_type_and_tag(
        root_set, types.MBENTITYSET, dagmc_tags['geom_dim'], [2])
    volume_range = my_core.get_entities_by_type_and_tag(
        root_set, types.MBENTITYSET, dagmc_tags['geom_dim'], [3])
