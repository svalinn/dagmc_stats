...
def get_dagmc_tags(core):
    ...
    dagmc_tags = {}
    dagmc_tags['geom_dim'] = core.tag_get_handle('GEOM_DIMENSION', size = 1, tag_type = types.MB_TYPE_INTEGER, #creates tag for the geo-
                                                 storage_type = types.MB_TAG_SPARSE, create_if_missing = True)# metric dimension
    
    dagmc_tags['category'] = core.tag_get_handle('CATEGORY', size = 32, tag_type = types.MB_TYPE_OPAQUE, #creates tag for the word of 
                                                 storage_type = types.MB_TAG_SPARSE, create_if_missing = True) #the category
    
    dagmc_tags['global_id'] = core.tag_get_handle('GLOBAL_ID', size = 1, tag_type = types.MB_TYPE_INTEGER, #creates tag for each entity
                                                  storage_type = types.MB_TAG_SPARSE, create_if_missing = True) #id

    return dagmc_tags


def get_entity_ranges(core, meshset, entity_types):
    """
    ...
    for entity_type in entity_types:
        entity_range[entity_type] = core.get_entities_by_type(meshset, entity_type) 
